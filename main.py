...
class Animal:  # klasa agentów
    ...
    def reproduce(self, agent):
        if self.ready_to_reproduce and len(agents) > 1:
            ...
            if agent_dis <= self.max_speed and agents[closest_agent_index].ready_to_reproduce:
                self.ready_to_reproduce = False
                self.health = 0.5
    ...
    def cross(self, parent):
        ...
        for i in range(len(self.dna)):
            ...
            if random.random() <= mutation_chance:
                new_dna[i] -= (self.dna[i] / random.uniform(0.75, 3))
                new_dna[i] += (self.dna[i] / random.uniform(0.75, 3))
...
foods_g = [Food(random.randrange(0 + move, width - move), random.randrange(0 + move, height - move), False) for _ in range(food_number)]
foods_p = [Food(random.randrange(0 + move, width - move), random.randrange(0 + move, height - move), True) for _ in range(poison_number)]
